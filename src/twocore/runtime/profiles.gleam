...
import twocore/runtime/instance.{type Binding, type Mode, Safe, safe_default}

/// The named Phase-1 **Safe** profile: the build-time `Binding` carrying the vetted
/// `twocore@runtime@rt_*` impl module names (unit 09). This is the ONLY profile Phase 1
/// ships, and it IS the fail-closed default (deny-all host, allowlisted BIFs, metering).
///
/// Returns the Safe `Binding` (`mode: Safe`). Total — never fails. There is deliberately no
/// `unsafe()` here: a caller cannot obtain an unsafe posture from this module (D4/D9).
pub fn safe() -> Binding {
  safe_default()
}
